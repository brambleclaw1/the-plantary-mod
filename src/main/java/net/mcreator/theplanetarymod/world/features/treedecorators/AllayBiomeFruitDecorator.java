...
public class AllayBiomeFruitDecorator extends CocoaDecorator {
	public static MapCodec<AllayBiomeFruitDecorator> CODEC = MapCodec.unit(AllayBiomeFruitDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, new ResourceLocation("the_planetary_mod:allay_biome_tree_fruit_decorator"), () -> DECORATOR_TYPE);
	}

	public AllayBiomeFruitDecorator() {
		super(0.2f);
	}
				...
				for (Direction direction : Direction.Plane.HORIZONTAL) {
					if (randomsource.nextFloat() <= 0.25F) {
						...
						if (context.isAir(blockpos)) {
							context.setBlock(blockpos, oriented(Blocks.SPRUCE_LEAVES.defaultBlockState(), direction1));
						}
					}
				}
			});
